...
def load_data(data_set_name, data_path='./data'):
    if data_set_name == 'UP':
        ...
        data = sio.loadmat(os.path.join(data_path, 'IP', 'Indian_pines.mat'))['indian_pines_corrected']
        labels = sio.loadmat(os.path.join(data_path, 'IP', 'Indian_pines_gt.mat'))['indian_pines_gt']
    elif data_set_name == 'SA':
        data = sio.loadmat(os.path.join(data_path, 'SA', 'Salinas.mat'))['salinas_corrected']
...
class HSIPatchDataset(Data.Dataset):
    def __init__(self, image, label_map, indexes, patch_length):
        """
        image: numpy array (H, W, C) normalized to [0, 1]
        label_map: numpy array (H, W) with -1 for unlabeled pixels
        indexes: list/array of linear indexes referencing original (unpadded) image
        """
        self.image = image.astype(np.float32)
        self.label_map = label_map.astype(np.int64)
        self.indexes = np.array(indexes)
        self.patch_length = patch_length
        self.height, self.width, self.channels = image.shape

        self.image_padded = data_pad_zero(self.image, patch_length)
        self.label_padded = np.pad(self.label_map,
    ...
    def __getitem__(self, idx):
        linear_idx = int(self.indexes[idx])
        pos_x = linear_idx // self.width + self.patch_length
        pos_y = linear_idx % self.width + self.patch_length

        x_start, x_end = pos_x - self.patch_length, pos_x + self.patch_length + 1
        y_start, y_end = pos_y - self.patch_length, pos_y + self.patch_length + 1

        data_patch = self.image_padded[x_start:x_end, y_start:y_end, :]
        label_patch = self.label_padded[x_start:x_end, y_start:y_end]

        data_patch = torch.from_numpy(np.transpose(data_patch, (2, 0, 1))).type(torch.FloatTensor)
        label_patch = torch.from_numpy(label_patch).type(torch.LongTensor)

        linear_idx_tensor = torch.tensor(linear_idx, dtype=torch.long)

        return data_patch, label_patch, linear_idx_tensor


def build_patch_dataloader(image, label_map, indexes, patch_length, batch_size, shuffle):
    if indexes is None or len(indexes) == 0:
        return None
    dataset = HSIPatchDataset(image=image,
                              label_map=label_map,
                              indexes=indexes,
...
def build_patch_dataloaders(image, label_map, index_tuple, patch_length, batch_size):
    train_loader = build_patch_dataloader(image, label_map, index_tuple[0], patch_length, batch_size, shuffle=True)
    val_loader = build_patch_dataloader(image, label_map, index_tuple[1], patch_length, batch_size, shuffle=False)
    test_loader = build_patch_dataloader(image, label_map, index_tuple[2], patch_length, batch_size, shuffle=False)
